 ...
 */
function getWebComponentsReadyCallback(memberName, callExpression) {
 ...
 */
function getHtmlImportsReadyCallback(memberName, callExpression) {
    const args = callExpression.arguments;
    if (args.length !== 1) {
        return;
    }
 ...
 */
function removeUnnecessaryEventListeners(program) {
    for (const nodePath of document_util_1.getTopLevelStatements(program)) {
        const statement = nodePath.node;
        if (statement.type !== 'ExpressionStatement' ||
            statement.expression.type !== 'CallExpression') {
            continue;
        }
        let memberName = document_util_1.getMemberName(statement.expression.callee);
        if (!memberName && statement.expression.callee.type === 'Identifier') {
            memberName = statement.expression.callee.name;
        }
        const callback = getWebComponentsReadyCallback(memberName, statement.expression) ||
...
exports.removeUnnecessaryEventListeners = removeUnnecessaryEventListeners;
//# sourceMappingURL=remove-unnecessary-waits.js.map