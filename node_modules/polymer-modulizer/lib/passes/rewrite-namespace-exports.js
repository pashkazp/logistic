...
function rewriteNamespacesAsExports(program, document, namespaces) {
    return new RewriteNamespaceExportsPass(program, document, namespaces).run();
}
exports.rewriteNamespacesAsExports = rewriteNamespacesAsExports;
class RewriteNamespaceExportsPass {
    constructor(program, document, namespaces) {
     ...
     */
    run() {
        for (const path of document_util_1.getTopLevelStatements(this.program)) {
            const statement = path.node;
            const namespaceDeclaration = getNamespaceDeclaration(statement, this.namespaces);
            const localNamespaceDeclaration = getNamespaceDeclaration(statement, this.localNamespaceNames);
     ...
     */
    rewriteDeclaration(nodePath, exportedExpression, fullyQualifiedNamePath) {
        ...
        if (this.isNamespace(nodePath.node) &&
            ...
            this.rewriteNamespaceObject(fullyQualifiedName, exportedExpression, nodePath);
        }
            ...
            // declarations
            this.rewriteAssignmentWithIdentifierRHS(exportedExpression, fullyQualifiedName, fullyQualifiedNamePath, nodePath);
        }
        else {
            ...
            const variableKind = this.mutableNames.has(correctedNamespaceName) ? 'let' : 'const';
            const newExportNode = jsc.exportNamedDeclaration(jsc.variableDeclaration(variableKind, [jsc.variableDeclarator(jsc.identifier(name), exportedExpression)]));
            replacePreservingComments(nodePath, newExportNode);
            this.exportMigrationRecords.push({ oldNamespacedName: correctedNamespaceName, es6ExportName: name });
        }
    }
    /**
     ...
     * into an ES6 export.
     */
    rewriteLocalDeclaration(path, value, memberPath) {
        ...
        replacePreservingComments(path, jsc.exportNamedDeclaration(jsc.variableDeclaration(this.mutableNames.has(fullyQualifiedName) ? 'let' : 'const', [jsc.variableDeclarator(jsc.identifier(nameExportedAs), value)])));
        this.exportMigrationRecords.push({ oldNamespacedName: fullyQualifiedName, es6ExportName: nameExportedAs });
    }
    /**
     ...
     */
    rewriteNamespaceObject(fullyQualifiedName, body, nodePath) {
        const namespaceExports = getNamespaceExports(body, this.mutableNames, fullyQualifiedName);
        ...
        if (nodePathComments.length > 0) {
            const message = `TODO(modulizer): A namespace named ${fullyQualifiedName} was\n` +
                `declared here. The surrounding comments should be reviewed,\n` +
                `and this string can then be deleted`;
            const tombstone = jsc.expressionStatement(jsc.templateLiteral([jsc.templateElement({ raw: message, cooked: message }, true)], []));
            tombstone.comments = nodePathComments;
        ...
        this.exportMigrationRecords.push({ oldNamespacedName: fullyQualifiedName, es6ExportName: '*' });
    }
    /**
     ...
     */
    rewriteAssignmentWithIdentifierRHS(identifier, fullyQualifiedName, fullyQualifiedNamePath, assignment) {
        const [namespaceFeature] = this.document.getFeatures({ id: fullyQualifiedName, kind: 'namespace' });
        if (namespaceFeature !== undefined) {
            ...
            const namespaceDeclarationStatement = document_util_1.getNodePathInProgram(this.program, namespaceFeature.astNode);
            if (namespaceDeclarationStatement == null) {
                throw new Error(`can't find associated node for namespace`);
            }
            const namespaceDeclarationValue = getAssignmentValue(namespaceDeclarationStatement.node);
            if (namespaceDeclarationValue === undefined ||
                namespaceDeclarationValue.type !== 'ObjectExpression') {
                return;
            }
            this.rewriteNamespaceObject(fullyQualifiedName, namespaceDeclarationValue, namespaceDeclarationStatement);
            ...
            let exportedName = fullyQualifiedNamePath[fullyQualifiedNamePath.length - 1];
            ...
            replacePreservingComments(assignment, jsc.exportNamedDeclaration(null, // declaration
            [jsc.exportSpecifier(identifier, jsc.identifier(exportedName))]));
            this.exportMigrationRecords.push({ es6ExportName: exportedName, oldNamespacedName: fullyQualifiedName });
        }
    }
    isNamespace(node) {
        const namespaces = this.document.getFeatures({ kind: 'namespace' });
        for (const namespace of namespaces) {
            if (document_util_1.isSourceLocationEqual(util_1.babelNodeToEstreeNode(namespace.astNode.node), node)) {
                return true;
            }
        }
        return false;
 ...
 */
function getLocalNamesOfLocallyDeclaredNamespaces(document) {
    const names = [];
    for (const namespace of document.getFeatures({ kind: 'namespace' })) {
        const astNode = util_1.babelNodeToEstreeNode(namespace.astNode.node);
 ...
 */
function getNamespaceExports(namespace, mutableNames, namespaceName) {
    const exportRecords = [];
    for (const propNode of namespace.properties) {
        ...
        if (value.type === 'ObjectExpression' || value.type === 'ArrayExpression' ||
            value.type === 'Literal') {
            const node = jsc.exportNamedDeclaration(jsc.variableDeclaration(isMutable ? 'let' : 'const', [jsc.variableDeclarator(key, value)]));
            node.comments = getCommentsFromNode(propNode);
            exportRecords.push({ name, node });
        }
        else if (value.type === 'FunctionExpression') {
            const func = value;
            const node = jsc.exportNamedDeclaration(jsc.functionDeclaration(key, // id
            func.params, func.body, func.generator));
            node.comments = getCommentsFromNode(propNode);
            exportRecords.push({ name, node });
        }
            ...
            const node = jsc.exportNamedDeclaration(null, [jsc.exportSpecifier(jsc.identifier(name), jsc.identifier(name))]);
            node.comments = getCommentsFromNode(propNode);
            exportRecords.push({ name, node });
        }
        else {
 ...
 */
function getNamespaceDeclaration(statement, namespaces) {
    ...
    const memberPath = document_util_1.getMemberPath(assignment.left);
    ...
    // `Polymer.foo = {}`
    const isAssignmentToNamespace = namespaces.has(memberPath.slice(0, -1).join('.'));
 ...
 */
function getMutableNames(program) {
    const mutable = new Set();
    const assignedOnce = new Set();
    astTypes.visit(program, {
        /** DeeplyNestedNamespace = 'something'; */
        visitAssignmentExpression(path) {
            const memberName = document_util_1.getMemberName(path.node.left);
        ...
        visitUpdateExpression(path) {
            const memberName = document_util_1.getMemberName(path.node.argument);
            if (memberName) {
                mutable.add(memberName);
            }
            this.traverse(path);
...
function replacePreservingComments(nodePath, replacement) {
...
function getCommentsFromNode(node) {
    const results = [];
    if (node.comments) {
...
function correctComments(comments) {
    const correctedComments = [];
    for (const existingComment of comments) {
        const comment = Object.assign({}, existingComment);
...
//# sourceMappingURL=rewrite-namespace-exports.js.map