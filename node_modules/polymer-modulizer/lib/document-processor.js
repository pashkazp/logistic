...
const remove_namespace_initializers_1 = require("./passes/remove-namespace-initializers");
...
class DocumentProcessor {
    constructor(document, originalPackageName, urlHandler, conversionSettings) {
        ...
        const relativeConvertedUrl = this.urlHandler.convertedUrlToPackageRelative(this.convertedUrl);
        this.convertedFilePath =
            this.urlHandler.packageRelativeConvertedUrlToConvertedDocumentFilePath(originalPackageName, relativeConvertedUrl);
    ...
    isInternalNonModuleImport(scriptImport) {
        ...
        const isInternalImport = this.urlHandler.isImportInternal(this.convertedUrl, newScriptUrl);
    ...
    prepareJsModule() {
        ...
        const convertedHtmlScripts = new Set();
        const claimedDomModules = new Set();
        let prevScriptNode = undefined;
        /**
         ...
         */
        let htmlCommentsBeforeFirstScriptNode;
        for (const script of this.document.getFeatures()) {
            ...
            const scriptProgram = recast.parse(scriptDocument.parsedDocument.contents).program;
            rewrite_toplevel_this_1.rewriteToplevelThis(scriptProgram);
            remove_toplevel_use_strict_1.removeToplevelUseStrict(scriptProgram);
            ...
            const localClaimedDomModules = this.inlineTemplates(scriptProgram, scriptDocument);
            ...
            if (script.astNode && script.astNode.language === 'html') {
                if (prevScriptNode !== undefined) {
                    const comments = document_util_1.getCommentsBetween(this.document.parsedDocument.ast, prevScriptNode, script.astNode.node);
                    document_util_1.attachCommentsToFirstStatement(comments, statements);
                }
                else {
                    htmlCommentsBeforeFirstScriptNode = document_util_1.getCommentsBetween(this.document.parsedDocument.ast, prevScriptNode, script.astNode.node);
                }
        ...
        const trailingComments = document_util_1.getCommentsBetween(this.document.parsedDocument.ast, prevScriptNode, undefined);
        document_util_1.attachCommentsToEndOfProgram(trailingComments, combinedToplevelStatements);
        this.insertCodeToGenerateHtmlElements(program, claimedDomModules);
        remove_namespace_initializers_1.removeNamespaceInitializers(program, this.conversionSettings.namespaces);
        return {
            program,
     ...
     */
    insertCodeToGenerateHtmlElements(program, claimedDomModules) {
        const ast = this.document.parsedDocument.ast;
        ...
        const elements = [
            ...head.childNodes.filter((n) => n.tagName !== undefined),
            ...body.childNodes.filter((n) => n.tagName !== undefined)
        ];
        const genericElements = document_util_1.filterClone(elements, (e) => {
            return !(generatedElementBlacklist.has(e.tagName) || claimedDomModules.has(e));
        });
        if (genericElements.length === 0) {
            return;
        }
        const statements = document_util_1.createDomNodeInsertStatements(genericElements);
        document_util_1.insertStatementsIntoProgramBody(statements, program);
    }
    /**
     ...
     */
    inlineTemplates(program, scriptDocument) {
        ...
        for (const element of elements) {
            ...
            if (element.sourceRange === undefined ||
                !polymer_analyzer_1.isPositionInsideRange(element.sourceRange.start, scriptDocument.sourceRange)) {
                continue;
            }
            ...
            if (!document_util_1.canDomModuleBeInlined(domModule)) {
                continue;
            }
            ...
            const templateLiteral = jsc.taggedTemplateExpression(jsc.memberExpression(jsc.identifier('Polymer'), jsc.identifier('html')), document_util_1.serializeNodeToTemplateLiteral(parse5.treeAdapters.default.getTemplateContent(template)));
            const nodePath = document_util_1.getNodePathInProgram(program, element.astNode);
            ...
            if (node.type === 'ClassDeclaration' || node.type === 'ClassExpression') {
                ...
                node.body.body.splice(0, 0, jsc.methodDefinition('get', jsc.identifier('template'), jsc.functionExpression(null, [], jsc.blockStatement([jsc.returnStatement(templateLiteral)])), true));
            }
            else if (node.type === 'CallExpression') {
                // A Polymer hybrid/legacy factory function element
                const arg = node.arguments[0];
                if (arg && arg.type === 'ObjectExpression') {
                    arg.properties.unshift(jsc.property('init', jsc.identifier('_template'), templateLiteral));
                }
            }
            else {
                console.error(`Internal Error, Class or CallExpression expected, got ${node.type}`);
            }
        }
     ...
     */
    addImportMetaToElements(program, scriptDocument) {
        const elements = scriptDocument.getFeatures({ 'kind': 'polymer-element' });
        for (const element of elements) {
            // This is an analyzer wart. There's no way to avoid getting features
            // from the containing document when querying an inline document. Filed
            ...
            const importMeta = jsc.memberExpression(jsc.identifier('import'), jsc.identifier('meta'));
            const node = nodePath.node;
            if (node.type === 'ClassDeclaration' || node.type === 'ClassExpression') {
                // A Polymer 2.0 class-based element
                const getter = jsc.methodDefinition('get', jsc.identifier('importMeta'), jsc.functionExpression(null, [], jsc.blockStatement([jsc.returnStatement(importMeta)])), true);
                ...
                const arg = node.arguments[0];
                if (arg && arg.type === 'ObjectExpression') {
                    arg.properties.unshift(jsc.property('init', jsc.identifier('importMeta'), importMeta));
                }
            }
            else {
                console.error(`Internal Error, Class or CallExpression expected, got ${node.type}`);
     ...
     */
    convertDocumentUrl(htmlUrl) {
        // TODO(fks): This can be removed later if type-checking htmlUrl is enough
        if (!util_1.isOriginalDocumentUrlFormat(htmlUrl)) {
            throw new Error(`convertDocumentUrl() expects an OriginalDocumentUrl string` +
        ...
        if (jsUrl.endsWith('shadycss/apply-shim.html')) {
            jsUrl = jsUrl.replace('shadycss/apply-shim.html', 'shadycss/entrypoints/apply-shim.js');
        }
        if (jsUrl.endsWith('shadycss/custom-style-interface.html')) {
            jsUrl = jsUrl.replace('shadycss/custom-style-interface.html', 'shadycss/entrypoints/custom-style-interface.js');
        }
        ...
        if (this.originalPackageName === 'polymer' && jsUrl === './polymer.html') {
            ...
            jsUrl = jsUrl.replace(/polymer\/polymer\.html$/, 'polymer/polymer-legacy.html');
        }
        // Convert any ".html" URLs to point to their new ".js" module equivilent
        jsUrl = util_2.replaceHtmlExtensionIfFound(jsUrl);
        return jsUrl;
    }
    /**
     ...
     */
    convertScriptUrl(oldUrl) {
        // TODO(fks): This can be removed later if type-checking htmlUrl is enough
        if (!util_1.isOriginalDocumentUrlFormat(oldUrl)) {
            throw new Error(`convertDocumentUrl() expects an OriginalDocumentUrl string` +
