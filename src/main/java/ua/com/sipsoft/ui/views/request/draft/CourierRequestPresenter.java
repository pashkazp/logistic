...
/**
 * The Class CourierRequestPresenter.
 * 
 * @author Pavlo Degtyaryev
 */
@Slf4j
public class CourierRequestPresenter extends VerticalLayout {
    ...
    private static final long serialVersionUID = 7317669420578190718L;

    private CourierRequest request;

    /**
     * Instantiates a new courier request presenter.
     *
     * @param request the request
     */
    public CourierRequestPresenter(CourierRequest request) {
	log.debug("Instantiates a new courier request presenter");
	this.request = request;
	Label fromFacility = new Label(request.getFromPoint().getFacility().getName() + SEMICOLON);
	Label fromPoint = new Label(request.getFromPoint().getAddress());
	Label author = new Label(request.getAuthor().getUsername());
	Label toFacility = new Label(request.getToPoint().getFacility().getName() + SEMICOLON);
	Label toPoint = new Label(request.getToPoint().getAddress());
	Label creationDate = new Label(request.getCreationDate()
		.format(DateTimeFormatter.ofLocalizedDateTime(FormatStyle.SHORT, FormatStyle.MEDIUM)));
//		Label creationDate = new Label(request.getCreationDate().format(formatter));
	Label description = new Label(request.getDescription());
	...
	iconInfo.addClickListener(event -> ConfirmDialog
		...
		.withMessage(new HistoryEventViever<>(new Grid<>(CourierRequestEvent.class),
