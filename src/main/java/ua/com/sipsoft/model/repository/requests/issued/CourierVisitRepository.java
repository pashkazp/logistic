...
public interface CourierVisitRepository extends JpaRepository<CourierVisit, Long> {
	...
	/**
	 * Gets the courier visit by issued rouet sheet id.
	 *
	 * @param id   the id
	 * @param sort the sort
	 * @return the courier visit by issued rouet sheet id
	 */
	@Query(" SELECT request "
			+ " FROM IssuedRouteSheet issued "
			+ " join issued.requests request "
			+ " WHERE issued.id = :sheetid ")
	List<CourierVisit> getCourierVisitByIssuedRouetSheetId(@Param("sheetid") Long id, Sort sort);
}
